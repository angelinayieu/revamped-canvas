...
export const NodePromptInput = forwardRef<NodePromptInputHandle, Props>(function NodePromptInput(
  ...
  const fanOutDesignCandidates = useCanvasStore((s) => s.fanOutDesignCandidates);
    ...
    <div className="relative nodrag min-h-[47px]">
      {attachments.length > 0 && (
        <div className="flex flex-wrap items-center gap-1.5 pb-1.5">
          ...
          <div className="ml-auto flex items-center gap-1 rounded-md border border-border bg-background px-1 py-0.5">
            <Sparkles size={11} className="text-primary" />
            <span className="text-[10px] text-muted-foreground">design fan-out</span>
            <select
              value={fanCount}
            ...
            <button
              ...
              onClick={() => {
                ...
                fanOutDesignCandidates(nodeId, fanCount, text, attachments);
              }}
              className="rounded bg-primary px-1.5 py-0.5 text-[10px] font-medium text-primary-foreground hover:opacity-90 cursor-pointer"
              title={`Fork ${fanCount} React/Tailwind candidates from this image+prompt`}
            >
              fork
            </button>
          </div>
