...
// rehype-highlight wraps code tokens in <span>s, and our search-highlighter
// wraps matches in <mark>s, so the `children` ReactMarkdown hands to the `code`
// component is usually an array of React elements — not a string. `String([elem])`
// would yield "[object Object]", which is what was breaking code blocks (and
// the live preview that reads from them). Walk the tree to recover the real text.
function reactChildrenToString(node: React.ReactNode): string {
  ...
  const components = useMemo((): Components => {
    const base: Components = {
      code(props) {
        ...
        const codeString = reactChildrenToString(children).replace(/\n$/, "");
