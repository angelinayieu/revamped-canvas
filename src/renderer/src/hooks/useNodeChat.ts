...
export function useNodeChat(nodeId: NodeId) {
  ...
  // Auto-submit on mount when a pending payload was queued (e.g. by a
  // design-candidate fan-out). One-shot: consumeAutoSubmit removes the entry,
  // so a StrictMode re-mount won't fire twice.
  useEffect(() => {
    const pending = storeApi.getState().consumeAutoSubmit(nodeId);
    if (!pending) return;
    void submit(pending.prompt, pending.attachments);
  }, [nodeId, submit, storeApi]);
